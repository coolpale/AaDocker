			...
			if [ ! -f "$php_ext_dir/"$ext".so" ]; then
				...
				case "$ext" in
					...
					'rdkafka')
			            ...
			            aaLog "Trying to install LibRdKafka: ${LIBRDKAFKA_VER}"
						$lib_kafka=${PHP_EXT_SRC}'/'${LIBRDKAFKA_VER}
						if [ ! -f "${lib_kafka}/Makefile" ]; then
						    aaLog "LibRdKafka: ${LIBRDKAFKA_VER} Not Exist! Downloading..."
						    v=${"${LIBRDKAFKA_VER}"/librdkafka-/}
					...
					'zookeeper')
                        ...
                        aaLog "Trying to install Zookeeper Lib: ${LIBZOOKEEPER_VER}"
                        $lib_zookeeper=${PHP_EXT_SRC}'/'${LIBZOOKEEPER_VER}
                        if [ ! -f "${lib_zookeeper}/src/c/configure" ]; then
                            cd ${PHP_EXT_SRC}
                            curl -sSL "http://www-us.apache.org/dist/zookeeper/${LIBZOOKEEPER_VER}/${LIBZOOKEEPER_VER}.tar.gz"
                            tar -zxvf "${LIBZOOKEEPER_VER}.tar.gz"
                            rm -f ${LIBZOOKEEPER_VER}.tar.gz
                        fi
					...
					'composer')
						cd ${PHP_EXT_SRC}
						if [ ! -f "${PHP_EXT_SRC}/composer-setup.php" ]; then
                    ...
                    'swoole')
                        ...
                        if [ ${PHP_EXTS_WITH_HTTP2} -eq 1 ]; then
                            for dependency in $(ls "${PHP_EXT_DEPENDENCIES}"); do
                                if [ -f "${PHP_EXT_DEPENDENCIES}/$dependency/configure" ]; then
                                    case "${dependency:0:7}" in
                                        nghttp2)
                                            aaLog "$dependency is installing..."
                                            cd "${PHP_EXT_DEPENDENCIES}/$dependency"
                                            ./configure
                                            make
                                            break
                                        ;;
...
grantPrivileges() {
	...
	[ ! -d "${PHP_LOG_DIR}" ] && mkdir -p "${PHP_LOG_DIR}"
