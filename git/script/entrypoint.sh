		...
		if [ ! -z "$hot_key" -a ! -f "$hot_key" ]; then    
			case ${hot_key##*/} in 
				ssh_host_rsa_key)
					...
					aaLog "ssh-keygen -P \"\" -t rsa -b $rsa_bits -f \"$hot_key\""
					ssh-keygen -P "" -t rsa -b $rsa_bits -f "$hot_key" >> "${ENTRYPOINT_LOG}" 2>&1
					;;
				ssh_host_dsa_key)
					aaLog "ssh-keygen -P \"\" -t dsa -b 1024 -f \"$hot_key\""
				...
				ssh_host_ecdsa_key)
					...
					aaLog "ssh-keygen -P \"\" -t ecdsa -b $ecdsa_bits -f \"$hot_key\""
				...
				ssh_host_ed25519_key)
					...
					aaLog "ssh-keygen -P \"\" -t ed25519 -b $ed25519_bits -f \"$hot_key\""
...
if [ ! -f "$lock_file" ]; then
	...
	chown -R -v ${GIT_USER}:${GIT_GROUP} "${GIT_LOG_DIR}"
