...
setConfig() {
    cnf_lock="${S_P_L_DIR}/mysql-cnf-"$(echo -n "${LOG_TAG}" | md5sum | cut -d ' ' -f1)
    if [ ! -f "$cnf_lock" ]; then
        conf='/etc/my.cnf'
        [ ! -f "$conf" ] && echo -e "[mysqld]\nserver-id=1" > "$conf"
        aaLog "Configuration file: $conf"
        if [ ! -z "${MYSQL_SERVER_ID}" ]; then
            aaLog "my.cnf server-id=${MYSQL_SERVER_ID}"
            sed -Ei "s/^(server-id)[\s=]*.+$/server-id=${MYSQL_SERVER_ID}/" "$conf"
        fi
        if [ ! -z "${MYSQL_BINLOG_IGNORE_DB}" ]; then
            sed -Ei "s/^\s*(binlog-ignore-db|binlog-do-db)/#&/" $conf
            if [ "${MYSQL_BINLOG_IGNORE_DB}" != '*' ]; then
                echo "binlog-ignore-db=${MYSQL_BINLOG_IGNORE_DB}" >> $conf
                #for db in $(echo "${MYSQL_BINLOG_IGNORE_DB}" | tr ',' "\n"); do
        ...
        if [ ! -z "${MYSQL_REPLICATE_IGNORE_DB}" ]; then
            sed -Ei "s/^\s*(replicate-ignore-db|replicate-binlog-do-db)/#&/" $conf
            if [ "${MYSQL_REPLICATE_IGNORE_DB}" != '*' ]; then
                echo "replicate-ignore-db=${MYSQL_REPLICATE_IGNORE_DB}" >> $conf
            fi
        fi

        if [ ! -z "${MYSQL_BINLOG_SLAVE_UPDATES}" ]; then
            sed -Ei "s/^\s*binlog-slave-updates/#&/" $conf
            bsu=echo ${MYSQL_BINLOG_SLAVE_UPDATES} | awk '{print tolower($0)}'
            if [ "$bsu" == 'on' -o "$bsu" == 'true' -o "$bsu" == '1' -o $bsu -eq 1 ]; then
                echo "binlog-slave-updates=ON" >> $conf
            fi
        fi

        aaLog 'touch {'$cnf_lock'}'
        touch $cnf_lock
    fi
}
	...
	if [ ! -d "$data_dir/mysql" ]; then
        ...
        if [ ! -z "${MYSQL_ROOT_PASSWORD}" ]; then
            ...
            if [ ! -z "${MYSQL_REP_USER}" -a ! -z "${MYSQL_REP_PASSWORD}" ]; then
				...
				MYSQL_REP_PRIV_LEVEL=${MYSQL_REP_PRIV_LEVEL:-'*.*'}
                echo "GRANT REPLICATION SLAVE ON ${MYSQL_REP_PRIV_LEVEL} TO '${MYSQL_REP_USER}'@'${MYSQL_REP_BIND_ADDRESS}' IDENTIFIED BY '${MYSQL_REP_PASSWORD}' ;"| "${mysql[@]}"
                echo 'FLUSH PRIVILEGES ;' | "${mysql[@]}"
            fi
